...
struct ResponsiveLayout
{
    ...
    {
        const float dpi = (logW > 0 && fbW > 0) ? static_cast<float>(fbW) / static_cast<float>(logW) : 1.0f;

        const auto next = FromSize(logW, logH, dpi);

        changed_ = (next.mode != mode) || (next.logicalW != logicalW) || (next.logicalH != logicalH) ||
                   (next.dpiScale != dpiScale) || (next.sidebarWidth != sidebarWidth);
    ...
    [[nodiscard]] static ResponsiveLayout FromSize(int w, int h, float dpi = 1.0f)
    {
        ...
        if (w < 600)
            l.mode = LayoutMode::Phone;
        else if (w < 1024)
            l.mode = LayoutMode::Tablet;
        else
            l.mode = LayoutMode::Desktop;
