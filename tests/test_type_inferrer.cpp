...
{
    const auto r = InferColumnType({"2024-01-15T10:30", "2023-12-31T23:59", "2000-06-01T00:00"});
...
{
    const auto r = InferColumnType({"2024-01-15 10:30", "2023-12-31 23:59", "2000-06-01 00:00"});
    CHECK(r.type == InferredType::DateTime);
}
...
TEST_CASE("InferColumnType: DateTime preferred over Date", "[inferrer][datetime]")
{
    ...
    const auto r = InferColumnType({"2024-01-15T10:30", "2024-01-16T11:00", "2024-01-17T12:00"});
    CHECK(r.type == InferredType::DateTime);
}
...
TEST_CASE("InferColumnType: large sample capped at 200", "[inferrer][perf]")
{
    ...
    many.reserve(500);
    for (int i = 0; i < 500; ++i)
        many.push_back(std::to_string(i));
...
{
    const std::vector<std::string> lines = {"id,name,score", "1,Alice,95.5", "2,Bob,87.0", "3,Charlie,91.2"};
...
{
    const std::vector<std::string> lines = {"id\tname\tscore", "1\tAlice\t95.5", "2\tBob\t87.0"};
    const auto                     r     = DetectDelimiter(lines);
    CHECK(r.delimiter == '\t');
    CHECK(r.confidence > 0.4f);
}
...
{
    const std::vector<std::string> lines = {"id|name|value|extra", "1|foo|bar|baz", "2|qux|quux|corge"};
    const auto                     r     = DetectDelimiter(lines);
    CHECK(r.delimiter == '|');
}
...
TEST_CASE("DetectDelimiter: quoted fields with commas inside", "[delimiter]")
{
    ...
    const std::vector<std::string> lines = {"id,name,note", "1,Alice,\"hello, world\"", "2,Bob,\"foo, bar, baz\""};
    const auto                     r     = DetectDelimiter(lines);
