...
std::expected<DataSourcePtr, std::string>
AdapterRegistry::CreateConnected(std::string_view name, const ConnectionParams& params)
{
    auto ds = Create(name);
    if (!ds) {
        std::string reg;
        ...
        return std::unexpected(std::format(
            ...
            name, reg.empty() ? "(none)" : reg));
    }
    if (auto r = ds->Connect(params); !r)
