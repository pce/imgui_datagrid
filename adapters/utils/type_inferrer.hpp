#pragma once
/// Type inference utilities for schema detection.
/// Pure C++23, zero external dependencies — safe in headless test binaries.
///
/// THREE UTILITIES
///   InferColumnType(samples)   → most specific type fitting ≥ 90 % of values
///   DetectDelimiter(lines)     → field separator from {',','\t',';','|',':'}
///   DetectHeader(first, second)→ estimates whether row 0 is a header row
...
[[nodiscard]] HeaderResult DetectHeader(
