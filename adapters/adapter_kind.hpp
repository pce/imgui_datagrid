...
enum class AdapterKind : std::uint8_t { SQLite, CSV, Filesystem };

[[nodiscard]] constexpr std::string_view name_of(AdapterKind k) noexcept
{
    switch (k) {
        case AdapterKind::SQLite:     return "sqlite";
        case AdapterKind::CSV:        return "csv";
        case AdapterKind::Filesystem: return "filesystem";
    }
    return "unknown";
}
